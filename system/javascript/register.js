function input_to_var() {
    ...
    // Eingaben validieren
    if (!validInput.test(username) || !validInput.test(password) || !validInput.test(passwordwdh)) {
        alert("Ungültige Zeichen im Benutzernamen oder Passwort.");
        return;
    }

    // Wenn alles gut ist, zeige die Werte
    alert("In Variable username: " + username + " || In Variable password: " + password + " || In Variable passwordwdh: " + passwordwdh);
}