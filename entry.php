<?php
// entry.php
// 1) Session muss schon in start.php gestartet sein
...
// 3) Erfolg & Fehler initialisieren (für Direkt-Render in start.php)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_entry'])) {
    ...
    if (empty($errors)) {
        // 6) Entry- und Day-Objekt erzeugen
        $entry = new Entry($title, $description, $startDate, $endDate);
        // vorhandene Tage aus Session holen
        $days = $_SESSION['days'] ?? [];
        ...
        // 7) neuen Eintrag hinzufügen und in Session speichern
        $day->addEntry($entry);
        $days[$startDate]        = $day;
        $_SESSION['days']        = $days;
        $success = true;
    }
    // 8) Ergebnis für die nächste Ausgabe merken
    $_SESSION['entry_success'] = $success;
    $_SESSION['entry_errors']  = $errors;
    // 9) Falls AJAX, JSON zurückgeben
